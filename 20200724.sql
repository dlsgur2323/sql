...
1. CONNECT BY LEVEL <, <= 정수
    : 시작행, 연결될 다음 행과의 조건이 없음
      ==> CROSS JOIN과 유사
2. START WITH, CONNECT BY : 일반적인 계층쿼리
...
전체 사원의 급여 순위가 자신보다 한단계 낮은 사람의 급여값을 5번째 컬럼으로 생성
(단, 급여가 같을경우 입사일자가 빠른사람이 우선순위);
...
도전해보기!
윈도우 안쓰고 실습 5 풀기
SELECT b.empno, b.ename, b.hiredate, b.sal ,c.sal lag_sal
FROM
    (SELECT a.*, ROWNUM rn
     FROM
        (SELECT empno, ename, hiredate, sal
    ...
    (SELECT ROWNUM+1 rn, sal
     FROM
        (SELECT empno, ename, hiredate, sal
         FROM emp
         ORDER BY sal DESC, hiredate)) c
WHERE b.rn = c.rn(+)
ORDER BY b.sal DESC, b.hiredate;
...
논리 모델 : 시스템에서 필요로 하는 엔터티(테이블), 엔터티의 속성, 엔터티간의 관계를 기술
            ...
            논리 모델에서는 데이터의 전반적은 큰 틀을 설계
물리 모델 : 논리 모델을 갖고 해당 시스템이 사용 할 데이터베이스를 고려하여 최적화된 테이블, 컬럼, 제약조건을 설계하는 단계
...
관계 차수 : 1-N, 1-1, N-N ==> 1:N으로 변경할 대상
            수직바, 까마귀발
관계 옵션 : mandatory(필수), optional(옵션) ㅇ표기
