...
제약조건 생성 방법 2번 : 테이블 생성시 컬럼 기술 이후 별도로 제약조건을 기술하는 방법
dept_test 테이블의 deptno컬럼을 대상으로 PRIMARY KEY 제약조건 생성
...
dept_test 테이블에 deptno가 동일한 값을 갖는 insert 쿼리 2개 생성하여 정삭적으로 동작하는지 테스트
...
UNIQUE 제약조건 : 해당 컬럼의 값이 다른 행에 나오지 않도록(중복되지 않도록) 데이터 무결성을 지켜주는 조건
                EX : 사번, 학번
...
CREATE TABLE dept_test (
    ...
    DNAME     VARCHAR2(14),
    LOC       VARCHAR2(13),
     CONSTRAINT uk_dept_test_dname UNIQUE (DNAME, LOC)  --> dname과 loc를 결합해서 중복되는 데이터가 없으면 됨.
                                                         --  ddit, daejeon <-중복-> ddit, daejeon
                                                         --  ddit, daejeon <-중복아님-> ddit, 대전
);

dname, loc 컬럼 조합으로 중복된 데이터가 들어가는지 안들어가는지 테스트

dname, loc 컬럼 조합의 값이 동일한 데이터인 경우 --> 에러(unique 제약조건에 의해)
...
*참조되는 테이블의 컬럼에는 (dept_test.deptno) 인덱스가 생성되어있어야 한다.
...
dept_test 테이블의 deptno컬럼을 참조하는 emo_test 테이블 생성
...
2. emp_test 테이블에 deptno 컬럼으로 dept_test.deptno 컬럼을 참조
...
CONSTRAINT 제약조건명 제약조건타입 (대상컬럼) REFERENCES 참조테이블(참조테이블의 컬럼명)
...
1. 존재하지 않았던 새로운 컬럼 추가
    ** 테이블의 컬럼 기술순서를 제어하는 건 불가
    ** 신규로 추가하는 컬럼의 경우 컬럼 순서가 항상 테이블의 마지막
    ** 설계를 할 때 컬럼 순서에 대해 충분히 고려, 누락된 컬럼이 없는지도 고려
...
ALTER TABLE emp_test DROP COLUM cell;
...
때가되면 다시 활동화 하여 데이터 무결성에 대한 부분을 강제할 수 있음
...
pk_emp_test가 비활성화 되었기 때문에 empno컬럼에 중복되는 값 입력이 가능
...
실습 2
emp와 dept 테이블 제약조건 3개 empno와 deptno 에 primary key emp.deptno는 foreign key
