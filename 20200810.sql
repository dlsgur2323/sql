...
BEGIN
    ...
    dt_sum := dt_sum + v_dt(i).dt - v_dt(i+1).dt;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('avgdt : ' || dt_sum / (v_dt.count-1));
END;
/
...
정해진 작업을 한 다음 결과를 돌려주는 PL/SQL block
문법
...
사번을 입력받아서(파라미터) 해당 사원의 이름을 반환하는 함수 getEmpName t생성
...
CREATE OR REPLACE PACKAGE 패키지명 AS
    FUNCTION 함수이름 (인자) RETURN 타입;
END 패키지명;
/

2. body(구현부)        : class
CREATE OR REPLACE PACKAGE BODY names AS
    FUNCTION 함수이름 (인자) RETURN 타입 IS
        --선언부
    BEGIN
        --실행부
        RETURN
...
2. users 테이블의 변경을 감지하고 실행할 트리거를 생성
...
CREATE OR REPLACE TRIGGER make_history
    ...
    BEGIN
        -- users 테이블의 특정행이 update가 되었을 경우 실행
        /*:OLD.컬럼명 ==> 기존값
          :NEW.컬럼명 ==> 갱신값*/
...
신규개발 : 좋아함
유지보수 : 안좋아함
            유지보수적인 측면에서는 문서화가 잘 안되어 있을 경우 코드 동작에 대한 이해가 힘들어 짐


예외 : EXCEPTION
    JAVA : exception, error
            - checked exception : 반드시 예외처리 해야하는 예외
            - unchecked exception : 예외처리를 안해도 되는 예외

PL/SQL : PL/SQL 블록이 실행되는 동안 발생한 에러
예외의 종류
