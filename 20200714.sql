...
emp 테이블에서 급여 정보인 sal, comm 컬럼을 제외하고 나머지 6개 컬럼만 조회할 수 있는 SELECT 쿼리를 v_emp 이름의 view로 생성
...
hr 계정으로 접속하여 테스트
v_emp view는 dlsgur 계정이 hr 계정에게 select 권한을 주었기 때문에 정상적으로 조회 가능
SELECT *
FROM DLSGUR.v_emp;
...
v_emp
    ...
    2. SELECT *
        FROM v_emp; 결과가 몇건일까? 16건
        view라고 하는 것은 실체가 없는 데이터 집합을 정의하는 sql이기 때문에 해당 sql에서 사용하는 테이블의 데이터가 변경이 되면 view에도 영향을 미친다.
    
view는 sql 이기 때문에 조인된 결과나, 그룹함수를 적용해서 행의 건수가 달라지는 sql도 view로 생성하는 것이 가능.
...
2. currval (current value)
    ...
    nextval 함수를 통해 얻을 값을 다시 확인 할 때 사용
...
emp 테이블의 pk_emp PRIMARY KEY 제약조건을 통해 EMPNO컬럼으로 인덱스 생성이 되어 있는 상태
...
2. pk_emp 인덱스를 이용하여 사용자가 원하는 행을 빠르게 찾아가서
   필요한 컬럼들은 인덱스에 저장된 rowid를 이용하여 테이블의 행으로 바로접근
...
1.NON-UNIQUE 인덱스 생성(중복 가능)
 ...
 NON-UNIQUE 명명규칙 : IDX_NU_테이블명_01;
